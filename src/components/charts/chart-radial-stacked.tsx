...
// Configuration for the chart with city labels and colors
const chartConfig = {
  newDelhi: {
    label: "New Delhi",
    color: "#6c4fed",
...
export function HalfDonutChart() {
  // Calculate total visitors based on provided city data
  const totalVisitors =
  ...
  // Data structure for city breakdown details
  const data = [
      ...
      <div className="flex flex-1 items-center pb-18">
        <ChartContainer config={chartConfig} className="mx-auto aspect-square w-full max-w-[250px]">
          <RadialBarChart data={chartData} endAngle={180} innerRadius={80} outerRadius={130}>
              ...
              <Label content={({ viewBox }) => (
                viewBox && "cx" in viewBox && "cy" in viewBox ? (
                  <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle">
                    <tspan x={viewBox.cx} y={(viewBox.cy || 0) - 16} className="fill-muted-foreground text-lg">Total</tspan>
                    <tspan x={viewBox.cx} y={(viewBox.cy || 0) + 30} className="fill-muted-foreground text-lg">
            ...
            {/* Individual Radial Bars */}
            <RadialBar dataKey="newDelhi" stackId="a" cornerRadius={5} fill="var(--color-newDelhi)" className="stroke-transparent stroke-2" />
            <RadialBar dataKey="mumbai" fill="var(--color-mumbai)" stackId="a" cornerRadius={5} className="stroke-transparent stroke-2" />
            <RadialBar dataKey="westBengal" fill="var(--color-westBengal)" stackId="a" cornerRadius={5} className="stroke-transparent stroke-2" />
            <RadialBar dataKey="others" fill="var(--color-others)" stackId="a" cornerRadius={5} className="stroke-transparent stroke-2" />
          </RadialBarChart>
        </ChartContainer>
      </div>
      
      {/* City Breakdown Details */}
